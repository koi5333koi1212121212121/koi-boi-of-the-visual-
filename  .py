...
def find_all_dats() -> list:
    ...
    This will discard ops that do not have a "file" parameter
    """
    external_dats = []

    # locates all DATs in project
    all_dats = root.findChildren(type=DAT)

    # reduce all dats to only those with a "file" par
    only_file_ops = [each for each in all_dats if each.pars("file")]
...
def Reload_tox_extension(comp):
    """Pulses the reinitextensions par for any COMP

    The intended use of this function is to reinit the extensions for any
    external op
    """
...
def Reload_tox(comp):
    """Pulses the reinitnet par for any COMP
